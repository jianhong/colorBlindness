...
### Use `cvdPlot` to check colors in a figure

Read the figures into R, then convert it into a taster grob and check the colors by `cvdPlot`.
...
`replacePlotColor` function is used to change the colors to safe colors.
...
img <- lapply(file.path(path, paste0("Plate", 1:24, ".png")), readPNG)
g <- lapply(img, rasterGrob, interpolate=TRUE)
null <- lapply(g, function(.ele) 
  grid.draw(cvdPlot(.ele, 
